...
- `SECRET_KEY` — сгенерируйте самостоятельно. Можно использовать [генератор](https://djecrety.ir/)
- `YANDEX_GEOCODER_API_KEY` — получите в [кабинете разработчика Яндекс.API](https://developer.tech.yandex.ru/). Ключ нужен для работы геокодера (преобразования адреса в координаты).
...
**Вариант 1: Автоматически через shell**
...
from star_burger.geocoding import fetch_coordinates

for restaurant in Restaurant.objects.all():
    coords = fetch_coordinates(restaurant.address)
    if coords:
        lon, lat = coords
        restaurant.longitude = float(lon)
        restaurant.latitude = float(lat)
        restaurant.save()
        print(f"✅ {restaurant.name}: {lat}, {lon}")
    else:
        print(f"❌ {restaurant.name}: адрес не найден")
```

**Вариант 2: Вручную через админку**

1. Зайдите в админку: `/admin/foodcartapp/restaurant/`
2. Откройте каждый ресторан
3. Введите координаты в поля "Широта" и "Долгота"
4. Сохраните

### Кэширование координат

Сайт сохраняет координаты адресов в таблицу `Place` (приложение `places`). Это ускоряет загрузку страницы менеджера, так как повторные запросы к геокодеру не выполняются.

Для просмотра сохраненных адресов зайдите в админку: `/admin/places/place/`
...
### Дополнительные настройки для prod-версии

- Убедитесь, что `DEBUG=False` в файле `.env`
